...
function (Track, Class, couchr, _, Spinner, scales, audio_controller) {
    var AudioTrack = Class.design('AudioTrack', {
        ...
        initialize : function(settings, chart_details){
            ...
            audio_emitter.on('seeking', spin);
            audio_emitter.on('loadstart', spin);
            audio_emitter.on('ended', spin);
            audio_emitter.on('progress', spin_off);
            me.spin_showing = false;

            function spin () {
                if (!me.spin_showing) 
                    me.spinner.spin(document.getElementById('audio-spinner'));
                me.spin_showing = true;
            }
